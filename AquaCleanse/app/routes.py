...
# -----------------------------
# Import necessary modules for web app, data handling, ML, email, and logging
#below are the rquired files that rae to be imported
...
# Gmail configuration (replace with real credentials for production)
GMAIL_EMAIL = "karthik2004raj@gmail.com"
GMAIL_PASSWORD = "mypasswordhere"

# -----------------------------
# DATABASE CONNECTIONS
# -----------------------------
# Import and initialize MongoDB collections for users, admins, and canal data
...
# -----------------------------
# Load the pre-trained CNN model for waste classification
...
# LOAD DATASET (CSV)
# -----------------------------
# Load water quality data from CSV for analysis and reporting
...
# UTILITY FUNCTIONS
# -----------------------------
# Function to send price quote emails using Gmail SMTP
...
# Estimate D5 (dissolved oxygen after 5 days) based on temperature
# Used for BOD (Biochemical Oxygen Demand) calculation
...
# Calculate BOD from initial and 5-day DO readings
def calculate_bod(d0, d5):
    return d0 - d5 if d0 > d5 else 0

# Predict diseases and solutions based on BOD value
...
# -----------------------------
# Import authentication functions (assumes auth.py exists)
...
# Decorator to require admin login for admin-only routes
...
# -----------------------------
# This function registers all routes to the Flask app
# Each route corresponds to a web page or API endpoint
...
def init_routes(app):
    ...
    # Logout route (clears user and admin sessions)
    @app.route('/logout')
    def logout():
    ...
    # Buy/Sell landing page (requires login)
    @app.route('/buy_sell')
    @login_required
    def buy_sell():
        return render_template('buy_sell.html')

    # Sell page (GET shows form, POST processes sale)
